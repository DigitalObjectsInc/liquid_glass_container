...
When the scope's subtree repaints, the scope re-records the backdrop (glass excluded) through a command-hashing canvas; the retained capture and its generation counter only change when the recorded content actually differs, so glass moving over a static backdrop reuses the previous capture. Content isolated behind descendant repaint boundaries (scroll viewports, list items, `RepaintBoundary`s, `FadeTransition`s) repaints without marking the scope dirty, so the scope also watches a signature of those boundaries' retained layers after each frame and recaptures on change — scrolling and boundary-isolated animations reach the refraction with at most one frame of latency. Each container caches a crop of that capture (sharp + Skia-gaussian-blurred, downscaled at large radii) keyed on generation and a grid-quantized crop rect, and composites the glass in one fragment-shader draw straight into the frame (`glass_main.frag`: refraction, dispersion, fresnel, glare, analytic interior shadow, LCH color math from the reference). The outer drop shadow is a mask-filtered superellipse path; edge anti-aliasing comes from a matching clip.
...
- Backdrop content that can't be recorded into pictures — platform views, textures, and retained composited layers (`BackdropFilter`, `ShaderMask`, `CompositedTransformFollower`) — is omitted from the refraction, and its presence disables capture reuse (a recapture per scope repaint). `FragmentShader` paints are captured correctly but also disable reuse. Composited opacity and image filters (`FadeTransition`, `ImageFiltered`) are reproduced; texture frames that update without a Dart-side repaint (video) can't be observed at all.
